...
class Day16A {
    private func run(path: String) -> String {
        ...
        var beams: [Beam] = []
        var visitedTile: [Position:[Direction]] = [:]
        let initialBeam = Beam(position: Position(x: 0, y: 0), direction: .right)
        beams.append(initialBeam)
        visitedTile[initialBeam.position] = [initialBeam.direction]
        ...
        while !beams.isEmpty {
            ...
            for beam in beams {
                ...
                for nextStep in nextSteps {
                    if let directions = visitedTile[nextStep.position] {
                        if !directions.contains(nextStep.direction) {
                            ...
                            visitedTile[nextStep.position] = tmpDirections
                        }
                    } else {
                        tmpBeams.append(nextStep)
                        visitedTile[nextStep.position] = [nextStep.direction]
                    }
                }
            }
        ...
        return String(visitedTile.keys.count)
    }
...
extension Day16A: AoCTest {
    func execute(test: Bool) -> String {
        if test {
            ...
            return run(path: "/Users/olahmate/aoc2023/tests/16.txt")
        }
    }
}
